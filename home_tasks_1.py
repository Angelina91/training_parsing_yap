...
actual_price = soup.find('span', attrs={'data-behavior': 'price-now'}).text
without_word = actual_price.split()
without_word.pop()
clean_price = ''.join(without_word)
actual_price = int(clean_price.replace(' ', ''))

# Посчитайте, сколько будет стоить пять комплектов богатырских доспехов.
total_sum = 5 * actual_price
